...
const contextMenuObjectName = computed(
  () => store.state.contextMenu.objectName
);
const contextMenuObjectColor = computed(
  () => store.state.contextMenu.objectColor
);
const contextMenuObjectBrackets = computed(
  () => store.state.contextMenu.objectBrackets
);
...
function itemNameHTML(item) {
  ...
  if (contextMenuObjectBrackets.value) {
    displayText = displayText.replace(
      "%s",
      `<span style="color:${contextMenuObjectColor.value};font-weight:600">[${contextMenuObjectName.value}]</span>`
    );
  } else {
    displayText = displayText.replace(
      "%s",
...
function handleItemClick(item) {
  ...
  if (cmd.substring(0, 5) === "wiki:") {
    ...
    wikiUrl = wikiUrl.replace(
      "%s",
      contextMenuObjectName.value.toLowerCase().replaceAll(" ", "-")
    );
